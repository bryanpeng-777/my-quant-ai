...
def generate_stop_loss_report(triggered_records, all_records_data):
    ...
    # 统计各市场股票数量
    us_count = sum(1 for r in triggered_records if r['market'] == MARKET_US)
    hk_count = sum(1 for r in triggered_records if r['market'] == MARKET_HK)
...
"""
    ...
    non_triggered_count = total_records - len(triggered_records)
    
    prompt = f"""
    你是资深价值投资分析师，擅长量化趋势分析和风险控制，熟悉美股和港股市场。
    
    以下是触发止损信号的股票列表（共 {len(triggered_records)} 只，包含 {market_summary}）：
    {all_stocks_text}
    ...
    3. 对每一只触发止损的个股分别进行如下操作：
       a. 首先列出当前关键值的数值，方便我去对比数据的正确性
       b. 说明买入日期、购买价格、购买数量（如有）、当前价格
       c. 说明跌幅百分比和亏损金额（如有）
       d. 明确说明本次买入已触发止损条件（跌幅 >= {STOP_LOSS_THRESHOLD}%）
       e. 给出明确的卖出建议（卖出本次买入的持仓数量）
    4. 最后给出所有触发止损股票的综合分析和操作建议
    5. 特别强调需要立即卖出的股票和对应的买入日期
    6. 注意：美股价格单位为美元($)，港股价格单位为港币(HK$)，请在报告中明确标注
    7. 本次监控共检查 {total_records} 条买入记录，其中 {len(triggered_records)} 条触发止损，{non_triggered_count} 条未触发
    """
...
def main():
    ...
    try:
        ...
        # 2. 如果有触发止损的股票，生成报告并发送邮件
        if triggered_records:
            print(f"[{datetime.now()}] 🔴 发现 {len(triggered_records)} 只股票触发止损信号")
            ...
            if report_content:
                # 3. 发送邮件通知
                subject = f"【止损卖出提醒】{len(triggered_records)}只股票触发止损信号 - {datetime.now().strftime('%Y-%m-%d')}"
                send_email(subject, report_content)
                print(f"[{datetime.now()}] ✅ 流水线执行成功，止损提醒已推送至邮箱。")
            else:
                print(f"[{datetime.now()}] ⚠️  AI报告生成失败，但止损信号已检测到")
        else:
            print(f"[{datetime.now()}] ✅ 所有持仓均未触发止损条件")
            print(f"[{datetime.now()}] 本次检查了 {len(all_records_data)} 只股票，均正常")
